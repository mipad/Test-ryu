...
#else
        private static IntPtr _signalHandlerPtr = IntPtr.Zero;
        private static MemoryBlock _codeBlock = null;
         ...
         public static void InitializeSignalHandler(Func<IntPtr, IntPtr, IntPtr> customSignalHandlerFactory = null)
        {
            ...
            lock (_lock)
            {
                ...
                if (OperatingSystem.IsLinux() || OperatingSystem.IsMacOS() || PlatformInfo.IsBionic)
                {
                    ...
                    {
                        _signalHandlerPtr = customSignalHandlerFactory(
                            UnixSignalHandlerRegistration.GetSegfaultExceptionHandler().sa_handler,
                            _signalHandlerPtr
                        );
                    }
