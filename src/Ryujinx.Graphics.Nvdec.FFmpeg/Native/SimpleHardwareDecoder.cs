// SimpleHardwareDecoder.cs - 使用统一的FFmpegApi
using System;
...
namespace Ryujinx.Graphics.Nvdec.FFmpeg.Native
{
    ...
    public class SimpleHardwareDecoder : IDisposable
    {
        ...
        public SimpleHardwareDecoder(SimpleHWCodecType codec, int width, int height)
        {
            ...
            bool hwAvailable = FFmpegApi.HardwareDecoderApi.hw_is_available();
            ...
            _handle = FFmpegApi.HardwareDecoderApi.hw_create(codec, width, height, true);
            Console.WriteLine($"hw_create returned: {_handle}");
            
            if (_handle == IntPtr.Zero)
            {
                IntPtr errorPtr = FFmpegApi.HardwareDecoderApi.hw_get_last_error(_handle);
        ...
        public bool Decode(byte[] data, out SimpleHWFrame frame)
        {
            ...
            int result = FFmpegApi.HardwareDecoderApi.hw_decode(_handle, data, data.Length, ref frame);
        ...
        public void Dispose()
        {
            if (!_disposed)
            {
                ...
                if (_handle != IntPtr.Zero)
                {
                    FFmpegApi.HardwareDecoderApi.hw_destroy(_handle);
                    _handle = IntPtr.Zero;
                }
                _disposed = true;
            }
    ...
    public static class PlatformHelper
    {
        ...
        public static bool IsHardwareDecoderAvailable 
        {
            get
            {
#if ANDROID
                try
                {
                    return FFmpegApi.HardwareDecoderApi.hw_is_available();
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error checking hardware decoder availability: {ex.Message}");
                    return false;
                }
#else
                return false;
