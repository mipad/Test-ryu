...
namespace Ryujinx.HLE.HOS.Applets.SoftwareKeyboard
{
    ...
    internal class SoftwareKeyboardRendererBase
    {
        ...
        private static SKBitmap LoadResource(Stream resourceStream, int newWidth, int newHeight)
        {
            ...
            {
                var resized = bitmap.Resize(new SKImageInfo(newWidth, newHeight), SKFilterMode.High);
                if (resized != null)
                {
                    bitmap.Dispose();
                    bitmap = resized;
        ...
        private void DrawTextBox(SKCanvas canvas, SoftwareKeyboardUIState state)
        {
            ...
            {
                Typeface = _labelsTextFont.Typeface,
                TextSize = _labelsTextFont.Size,
                IsAntialias = true,
                Color = _textNormalColor
            };
            ...
            if (state.TypingEnabled && cursorVisible)
            {
                ...
                else
                {
                    ...
                    {
                        Typeface = _inputTextFont.Typeface,
                        TextSize = _inputTextFont.Size,
