...
namespace ARMeilleure.Translation.Cache
{
    static partial class JitCache
    {
        ...
        private const int FullCacheSize = 256 * 1024 * 1024;
            ...
            lock (_lock)
            {
                ...
                if (OperatingSystem.IsWindows())
                {
                    JitUnwindWindows.InstallFunctionTableHandler(
                        firstRegion.Pointer, (ulong)_cacheSize, firstRegion.Pointer + Allocate(_pageSize)
                    );
                }
        ...
        public static IntPtr Map(CompiledFunction func)
        {
            ...
            lock (_lock)
            {
                ...
                if (Optimizations.CacheEviction)
                {
                    ...
                    _jitRegions[_activeRegionIndex].ExpandIfNeeded((ulong)funcOffset + (ulong)codeSize);
                }
                else
                {
            ...
            lock (_lock)
            {
                ...
                foreach (var region in _jitRegions)
                {
                    if (pointer.ToInt64() < region.Pointer.ToInt64() ||
                        pointer.ToInt64() >= (region.Pointer + _cacheSize).ToInt64())
                    {
                        continue;
                    }
