...
public:
    ...
    /**
     * Add an item to the back of the queue (move version)
     *
     * @param item - The item to add
     * @return true if item was added, false if the queue was full
     */
...
/**
 * Lock-free object pool for audio blocks
 */
...
public:
    ...
    std::unique_ptr<T> acquire() {
        ...
        // If pool is empty, create new object (shouldn't happen in normal operation)
    ...
    bool release(std::unique_ptr<T> obj) {
        if (obj) {
            obj->clear(); // Reset the object state
