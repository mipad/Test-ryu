...
class MainActivity : BaseActivity() {
    ...
    private val reattachWindowWhenReady = object : Runnable {
        override fun run() {
            ...
            // 使用安全的重新附着逻辑
            try {
                if (mainViewModel?.rendererReady == true && isGameRunning) {
    ...
    override fun onStop() {
        ...
        if (isGameRunning) {
            ...
            // 安全分离窗口
            try { mainViewModel?.gameHost?.safeDetachWindow() } catch (_: Throwable) {}
        }
        // 重要：绑定安全解除（防止泄漏）
    ...
    private fun hardColdReset(reason: String) {
        ...
        try { mainViewModel?.gameHost = null } catch (_: Throwable) {}
    }
