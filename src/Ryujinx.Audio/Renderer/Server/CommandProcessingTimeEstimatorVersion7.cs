...
namespace Ryujinx.Audio.Renderer.Server
{
    ...
    /// Implements performance improvements for key DSP commands in firmware 15.0.0+.
    /// Only overrides commands known to be virtual in Version5.
    /// </summary>
    public class CommandProcessingTimeEstimatorVersion7 : CommandProcessingTimeEstimatorVersion5
    {
        ...
        public override uint Estimate(DelayCommand command)
        {
            ...
            if (command.Enabled)
            {
                return command.Parameter.ChannelCount switch
                {
                    ...
                    2 => 36197,   // REV15 优化
                    4 => 68750,   // REV15 优化
                    6 => 118040,  // REV15 优化（修正：原为 11804，应为 ~118000）
                    _ => throw new NotImplementedException($"{command.Parameter.ChannelCount}")
                };
            }
        ...
        public override uint Estimate(Reverb3dCommand command)
        {
            ...
            if (SampleCount == 160)
            {
                ...
                return command.Parameter.ChannelCount switch
                {
                    1 => (uint)685.00f,  // REV15 优化（统一为 float）
